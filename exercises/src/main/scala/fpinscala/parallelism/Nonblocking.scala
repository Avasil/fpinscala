...
object Nonblocking {
  ...
  object Par {
    ...
    def choiceN[A](p: Par[Int])(ps: List[Par[A]]): Par[A] =
      es => new Future[A] {
        def apply(cb: A => Unit): Unit =
          p(es) { result =>
            eval(es)(ps(result)(es)(cb))
          }
      }

    def choiceViaChoiceN[A](a: Par[Boolean])(ifTrue: Par[A], ifFalse: Par[A]): Par[A] =
      choiceN(a map (res => if (res) 0 else 1))(List(ifTrue, ifFalse))
  ...
  def testChoiceN = {
    ...
    val p: Par[Int] = Par.unit(0)
    val listOfP: List[Par[Int]] = List(Par.unit(0), Par.unit(1), Par.unit(2))
    val mapOfP: Map[Int, Par[Int]] = Map(0 -> Par.unit(0), 1 -> Par.unit(1))

    println("run: " + Par.run(es)(Par.choiceN(p)(listOfP)))
    println("run: " + Par.run(es)(Par.choiceMap(p)(mapOfP)))
  }
}
